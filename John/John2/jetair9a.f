c jetair9a.f has corrections for ifread = 0 for priginal densities of 1.0
c jetair9.f has boundary set with top plane hole equals to volts
c jetair8d.f has gamma=1.0E-6 W max = 1.0E7 alpha max = 100
c jetair8d.f has 3 extra poiss calls per mm2 iteration
c jetair8 has three extra radialpoints to keep zero voltage at wall j = nrp3
...
c-----------------------------------------------------------------------
     ...
     r  an(0:mr), ae(0:mr,0:mz),f(0:mr),g(0:mz),vol(0:mr,0:mz),pinl,
     i  radius,volts,nr,nrp,nrp3, nrm, nz, nzp, nzm,nri,nro,
     i  nzc, nrc, nnbot, nra,nnrad,ngapup,
...
c----------------------------------------------------------------
  ...
  54  continue
      ...
      if (ifread .eq. 2) then
         ...
         rewind 304
         read(304,526) time
         read(304,526) ((rni(j,i),j=1,nrp3),i=1,nz)
         read(304,526) ((rne(j,i),j=1,nrp3),i=1,nz)
         read(304,526) ((rnp(j,i),j=1,nrp3),i=1,nz)
         read(304,526) ((rnm(j,i),j=1,nrp3),i=1,nz)
         read(304,526) ((phi(j,i),j=1,nrp3),i=1,nz)
         close(304)
      endif
c-----------------------------------------------------------------
...
c 506  format('  hail')
 ...
 852  format(1x,'iteration no.',i6,'  time =',e12.4,
     r '  max electron density =',1pe9.2,' relphi =',e12.4)
...
c       write(5,227) volts
 ...
 780  continue
      ...
      do 778 i=0,nzp
         write(5,510) i,z(i)
c         write(5,572) (phi(j,i),j=0,nrp)
         write(5,572) (phi(j,i),j=0,nrp4)
 778  continue
         write(5,501)
         write(5,508)
      do 792 i=0,nzp
         write(5,510) i,z(i)
         write(5,572) (ez(j,i),j=0,nrp4)
c         write(5,572) (ez(j,i),j=0,nrp)
 792  continue
         write(5,501)
         write(5,509)
      do 781 i=0,nzp
         write(5,510) i,z(i)
         write(5,572) (er(j,i),j=0,nrp4)
c         write(5,572) (er(j,i),j=0,nrp)
 781  continue
...
c          if (wire.ge.10000000.) wire = 10000000.
c	  if (wire.le.-10000000.) wire = -10000000.
          if (wire.ge.1000000.) wire = 1000000.
          if (wire.le.-1000000.) wire = -1000000.
          if (wir.ge.100000.) wire = 100000.
          if (wir.le.-100000.) wire = -100000.
...
c Keep charge densities in pin zero.
 ...
 191  continue
 190  continue
c Keep charge densities in external glass tube zero.
      do 212 i=0,nzp
      do 213 j=nr+1,nrp3
          dne(j,i)= 0.0
          dni(j,i)= 0.0
...
c	 rnet(j,i) = rnp(j,i) - rne(j,i)
 ...
 198  continue
      ...
      do 214 j=nr+1,nrp3
          ...
          rnet(j,i) = 0.0
          rnm(j,i) = 0.0
c          rni(nr+1,i) = rni(nr,i)
c          rne(nr+1,i) = rne(nr,i)
c          rne(nr+2,i) = rne(nr+1,i)
c          rnp(nr+1,i) = rnp(nr,i)
c          rnet(nr+1,i) = rnet(nr,i)
c          rnm(nr+1,i) = rnm(nr,i)
c          rni(0,i) = rni(1,i)
c          rne(0,i) = rne(1,i)
c          rnp(0,i) = rnp(1,i)
c          rnet(0,i) = rnet(1,i)
c          rnm(0,i) = rnm(1,i)
 ...
 199  continue
      time = time + delt
      write(*,852) m,time,rmaxne,relphi
...
c     end internal iteration
      ...
      open(304,file='arcfnout')
        rewind 304
         write(304,526) time
         write(304,526) ((rni(j,i),j=1,nrp3),i=1,nz)
         write(304,526) ((rne(j,i),j=1,nrp3),i=1,nz)
         write(304,526) ((rnp(j,i),j=1,nrp3),i=1,nz)
	write(304,526) ((rnm(j,i),j=1,nrp3),i=1,nz)
	write(304,526) ((phi(j,i),j=1,nrp3),i=1,nz)
      close(304)
 ...
 788  continue
      ...
      do 785 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rni(j,i),j=0,nrp4)
c         write(5,572) (rni(j,i),j=0,nr+1)
 785  continue
         write(5,501)
         write(5,522)
      do 786 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rne(j,i),j=0,nrp4)
c         write(5,572) (rne(j,i),j=0,nr+1)
 786  continue
         write(5,501)
         write(5,523)
      do 787 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rnp(j,i),j=0,nrp4)
c         write(5,572) (rnp(j,i),j=0,nr+1)
 787  continue
         write(5,501)
         write(5,820)
      do 798 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rnm(j,i),j=0,nrp4)
c         write(5,572) (rnm(j,i),j=0,nr+1)
 798  continue
         write(5,501)
         write(5,524)
      do 795 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rnet(j,i),j=0,nrp4)
c        write(5,572) (rnet(j,i),j=0,nr+1)
 795  continue
         write(5,501)
         write(5,508)
      do 779 i=0,nz
 ...
 789  continue
	 ...
	 mm8 = (nzc+nnbot)/2
      do 790 i=1,nz
         write(5,510) i,z(i)
         write(5,572) (eabs(j,i),j=1,nrp4)
 790  continue
         write(5,501)
         write(5,503)
      do 782 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rjnez(j,i),j=0,nrp4)
 782  continue
         write(5,501)
         write(5,504)
      do 783 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rjner(j,i),j=0,nrp4)
 783  continue
         write(5,501)
         write(5,505)
      do 816 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rjnpz(j,i),j=0,nrp4)
 816  continue
         write(5,501)
         write(5,519)
      do 817 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rjnpr(j,i),j=0,nrp4)
 817  continue
         write(5,501)
         write(5,527)
      do 819 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rjniz(j,i),j=0,nrpr)
 819  continue
         write(5,501)
         write(5,528)
      do 814 i=0,nz
         write(5,510) i,z(i)
         write(5,572) (rjnir(j,i),j=0,nrp4)
 814  continue
       close(5)
c files for MATLAB
      open(21,file='z.dat')
...
c 776  continue
c  17      call solve(1,nr,1,nz,de,dw,dn,ds,dc,df,phi,1,1)
      nr = nrp3
          call solve(1,nr,1,nz,de,dw,dn,ds,dc,df,phi,1,1)
      nr = nrp3-3
      do 775 i=0,nzp
            phi(nrp4,i) = phi(nrp3,i)
            phi(0,i) = phi(1,i)
...
c electric field from space charge - valid for Poisson equation, not current continuity
	 ...
	 df(j,i)=(e/epsilon)*(rnp(j,i)-rne(j,i)-rni(j,i))
         dn(j,i) = 2/(dz(i)*(dz(i)+dz(i+1)))
         ds(j,i+1) = dn(j,i)
	 de(j,i) = 2/(dr(j)*(dr(j)+dr(j+1)))
         dw(j,i+1) = de(j,i)
  21  continue
  20  continue
      do 80 i=1,nz
...
c         df(j,1) = 0.0
  ...
  101 continue
c upper plane at hole equals to volts
      do 509 j=1,nri
         dc(j,1) = dc(j,1) - dn(j,1)
         dn(j,1) = 0.0
         ds(j,1) = 0.d0
         ...
         df(j,1) = volts
  509 continue  
c upper plane at z = 0 at external glass wall phi=0.0
      do 103 j=nrp,nrp3
         dc(j,1) = dc(j,1) - dn(j,1)
         dn(j,1) = 0.0
         ds(j,1) = 0.d0
...
c of the last cell in the cathode.
 ...
 32	continue
c---------------------------------------------------------------------
c radial mesh, outer glass wall; 3 points of equal thickness dr(nrp) 
      	write(20,*) 'mesh8'
	do 34 j= nrp+1,nrp3+1
	   dr(j) = dr(nrp)
	   rb(j) = rb(j-1) + dr(j)
 34	continue
c---------------------------------------------------------------------
...
c-------------------------------------------------------
     ...
     r  dr(0:mr), dz(0:mz), r(0:mr), z(0:mz),rr(0:mr),drr(0:mr),
     r  an(0:mr),ae(0:mr,0:mz),f(0:mr),g(0:mz),vol(0:mr,0:mz),pinl,
     r  radius,volts,nr,nrp,nrp3, nrm, nz, nzp, nzm,nri,nro,
     r  nzc, nrc, nnbot, nra,nnrad,ngapup,
     r  irca(0:mz), nrca(0:mz), ica(0:mr,0:mz),
     r  iran(0:mz), nran(0:mz)
        do 9 i=0,mz
        do 10 j=0,mr
	ica(j,i)=0
 10      continue
