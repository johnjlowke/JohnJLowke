...
c          if (wir.le.-100000.) wire = -100000.
          if(er(j,i).le.0.0) then
              ...
              rjner(j,i) = wire*rne(j,i)-diff*(rne(j+1,i)-rne(j,i))/
     r	          dr(j)
              rjnpr(j,i) = -wir*rnp(j+1,i)
          else
              rjnir(j,i) = wir*rni(j+1,i)
              rjner(j,i) = wire*rne(j+1,i)-diff*(rne(j+2,i)-rne(j+1,i))/
     r	          dr(j+1)
...
c At glass boundary no emission or absorption of particles.
          if (j.eq.nr) then
             rjnez(j,i) = 0.0
	  if (i.eq.58.and.j.eq.5) write(*,*) rjnez(5,58)
             rjner(j,i) = 0.0
             rjniz(j,i) = 0.0
             rjnir(j,i) = 0.0
             rjnpz(j,i) = 0.0
...
c      do 50 j=1,nr
c           call zerocoef(dc,df,de,dw,dn,ds,volt,j,1)
c 50   continue
      e = 1.60206d-19
      epsilon = 8.85d-14
      mu = 10
