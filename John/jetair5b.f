c correction for fluxes at j=nr when ne(j+1,i) is called, line 368
c no backward differences
...
c          if(er(j,i).le.0.0) then
           if(j.ne.nr) then
              rjnir(j,i) = wir*(rni(j,i)+rni(j+1,i))/2
              rjner(j,i)=wire*(rne(j,i)+rne(j+1,i))/2
     r	          -diff*(rne(j+1,i)-rne(j,i))*2/(dr(j+1)+dr(j))
              rjnpr(j,i) = -wir*(rnp(j,i)+rnp(j+1,i))/2
           else
              rjnir(j,i) = wir*(rni(j,i)+rni(j,i))/2
              rjner(j,i)=wire*(rne(j,i)+rne(j,i))/2
              rjnpr(j,i) = -wir*(rnp(j,i)+rnp(j,i))/2
           endif
c          else
...
c At pin surfaces charges can flow in but not out.
        ...
        if (wize.le.-10000000.) wize = -10000000.
          if (wiz.ge.100000.) wiz = 100000.
          if (wiz.le.-100000.) wiz = -100000.
c        if(ez(j,i).le.0.0) then
         if(i.ne.nz) then
            rjniz(j,i) = wiz*(rni(j,i)+rni(j,i+1))/2
            rjnez(j,i) = wize*(rne(j,i)+rne(j,i+1))/2
     r	          -diff*(rne(j,i+1)-rne(j,i))/dz(i)
            rjnpz(j,i) = -wiz*(rnp(j,i)+rnp(j,i+1))/2
         else
            rjniz(j,i) = wiz*(rni(j,i)+rni(j,i))/2
            rjnez(j,i) = wize*(rne(j,i)+rne(j,i))/2
            rjnpz(j,i) = -wiz*(rnp(j,i)+rnp(j,i))/2
          endif
c        else
c       rjniz(j,i) = wiz*rni(j,i+1)
c       rjnez(j,i) = wize*rne(j,i+1)
c     r	          -diff*(rne(j,i+1)-rne(j,i))/dz(i)
c        rjnpz(j,i) = -wiz*rnp(j,i)
c      endif
...
c          if (eon.le.3.0E-17) att = rne(j,i)*3.75E6
