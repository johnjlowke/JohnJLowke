c rr23.f has fill for electron velocities; Patankar formulation
...
c--------------------------------------------------------------------------------
c     subroutine fillelec
c     this subroutine calculates the coefficients needed for Patankar coefficients
c         for electron axial velocity
c------------------------------------------------------------------------------
      subroutine fillelec
      parameter(mr=200, mz=100, nsh=21)
      dimension fe(0:mr,0:mz),fw(0:mr,0:mz),fn(0:mr,0:mz),fs(0:mr,0:mz)
  ...
  15  continue
      do 20 i=1,nz
      do 22 j=0,nrm
         temp2 = 0.5*(rne(j,i)*vr(j,i)*ae(j,i) +
     1               rne(j+1,i)*vr(j+1,i)*ae(j+1,i))
         if (j .ne. 0) then
            fe(j,i) = temp2
         end if
         fw(j+1,i) = temp2
  ...
  20  continue
      do 30 i=0,nz
      do 31 j=1,nrm
         temp2 = 0.5*(rne(j,i)*vz(j,i)*an(j) +
     1               rne(j+1,i)*vz(j+1,i)*an(j+1))
         if (j .eq. 1) temp2 = temp2 + 0.5*rne(1,i)*vz(1,i)*an(1)
         if (i .ne. 0) then
            fn(j,i) = temp2
         end if
         fs(j,i+1) = temp2
...
c
      do 50 i=0,nzm
      do 51 j=1,nr
         temp2 = 0.5*(rne(j,i)*vz(j,i)+rne(j,i+1)*vz(j,i+1))*an(j)
         if (i .ne. 0) then
            fn(j,i) = temp2
         end if
         fs(j,i+1) = temp2
...
c but this formulation does notinclude convection of electrons by the electric field
c----------------------------------------------------------------------
      subroutine fillde
      implicit double precision (a-h,o-z)
      logical rvpol,actest
