c rr22e.f has "+2" for Patankar coefficients
c rr22d.f has mofied backward boundary conditions
c rr22c.f removes attachment from rnc rr22c.f removes attachment from rnee
...
c diffusion omitted to avoid complexity
          ...
          rece = 0.0
          reci = 0.0
c Radial fluxes with backward differences
       do 193 i=1,nz
       do 194 j=1,nr
          wir = rmob*er(j,i)
          wire = 100*wir
     ...
     r	          (dr(j+1)+dr(j))
              rjnpr(j,i) = -wir*rnp(j,i+2)
          else
              rjnir(j,i) = wir*rni(j+2,i)
              rjner(j,i)=wire*rne(j+2,i)-diff*(rne(j+1,i)-rne(j,i))*2/
     r	          (dr(j+1)+dr(j))
              rjnpr(j,i) = -wir*rnp(j,i)
          endif
          if (j.le.1) rjner(j,i) = 0.0
c Axial fluxes with backward differences
c        wiz = -rmob*ez(j,i)
        wiz = rmob*ez(j,i)
        wize = 100*wiz
...
c           rjniz(j,i) = wiz*rni(j,i+1)
           rjniz(j,i) = wiz*rni(j,i+2)
c           rjnez(j,i) = wize*rne(j,i+1)
           rjnez(j,i) = wize*rne(j,i+2)
