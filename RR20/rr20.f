...
c Boundary condition for phi for z = 0 cm
          if (time.gt.timezero) then 
            nrrvtest = 1
	    voltcz = (time - timezero)*rrrv
          else
             nrrvtest = 0
             do 301 j=0,nrp
                 phi(j,1) = voltcz*(time - timezero)/timezero
 301         continue
          endif
...
c Start initial calculaions with poiss
      do 200 m=1,mm
        itit=itit+1
	mc = m
         imc=istore*int(float(mc)/float(istore))
         if (imc.eq.mc) mc=mm+1
        call amf(lte)
        if(itoor.eq.1)then
             write(*,*)' 1             temp>30,000, stop'
             go to 461
        endif
...
c electric field from space charge effects at lower electrode
         if (i.ge.60.and.i.le.71) then
           phi(j,i) = (e/epsilon)*(rnp(j,i)-rne(j,i)-rni(j,i))*vol(j,i)
         endif
         call poiss(curr,voltcz,nrrvtest)
  ...
  812  continue
          ...
          if (jscreen.eq.1) write(*,991)
 991    format(1x,' <solving for t>')
         call ener(itoor,itor,itoz,htoor,irst)
        if(itoor.eq.1) then
        go to 461
        endif
         volt = phi(1,1)
...
c values of current and voltage with time
      ...
      write(*,226) n, time, curr, voltp
      write(*,181) curr,time,n,voltp
      write(305,181) curr,time,n,voltp
...
c             nrrvtest = 1
c             voltcz = voltp 
c             call poiss(curr,voltcz,nrrvtest)
...
c
      write(*,1072) deltel
      write(*,226) n, time, curr, volt
      if (mc.ge.mm1.or.resmax.le.rsdl)  write(*,226)n, time, curr, volt
        fname='record'
      open(5,file=fname)
      rewind 5
      if (lte.eq.1) then
...
c      do 788 i=nza,nzp
 ...
 777  continue
         write(*,1072)  sig(10,85),sig(53,85)
         write(5,501)
         write(5,506)
      do 782 i=0,nzp
         write(5,510) i,z(i)
...
c 783  continue 
 ...
 555  format(1x,'arc voltage =',f10.2,'v arc power =',f10.1,'w')
      ...
      if (mc.ge.mm1) write(5,556) (hetr(j),j=1,nr)
      if (jscreen.eq.1) write(*,557) hanode, hside,  htot
      if (mc.ge.mm1) write(5,557) hanode, hside,  htot
...
c        do 18 j=1,nr
c        if (j .ne. nr) then
c          tee(j,i)=ae(j,i)/(dc(j,i)-aen(j,i)-aes(j,i)-aew(j,i)-aee(j,i))
c        else
c          tee(j,i) = 0.
c        end if
c  18    continue
...
c-----------------------------------------------------------------------
      subroutine poiss(curr,voltcz,nrrvtest)
      implicit double precision (a-h,o-z)
      logical rvpol
      parameter(mr=200, mz=100, nsh=21)
      dimension cd0(0:mr)
...
c include space charge shieth field at lower electrode
c       do 20 i=0,nz
c      do 21 j=1,nr
...
c---solve matrix
  17      call solve(1,nr,1,nz,de,dw,dn,ds,dc,df,phi,0,1)
...
c----------------------------------------------------------------------
      subroutine fill(cd0,voltcz,nrrvtest)
      implicit double precision (a-h,o-z)
      logical rvpol
      parameter(mr=200, mz=100, nsh=21)
      dimension cd0(0:mr)
...
c
      do 10 i=1,nz
      do 11 j=1,nr
         dife = 2.*ae(j,i)*sigr(j,i)/(dr(j)+dr(j+1))
         de(j,i) = dife
         dw(j+1,i) = dife
         difn = 2.*an(j)*sigz(j,i)/(dz(i)+dz(i+1))
         dn(j,i) = difn
         ds(j,i+1) = difn
c inclusion of diffusion current
         df(j,i) = ae(j,i)*cder(j,i)-ae(j-1,i)*cder(j-1,i)
     +            +an(j)*cdez(j,i)-an(j)*cdez(j,i-1)
         df(j,i) = -df(j,i)
 11   continue
 10   continue
      do 30 j=1,nr
...
c Top boundary condition
      do 50 j=1,nr
      if (nrrvtest.eq.0) then
           df(j,1) =cd0(j)*an(j)
      else
	   call zerocoef(dc,df,de,dw,dn,ds,voltcz,j,1)
      endif
           dc(j,nz) = dc(j,nz) + 2.*an(j)*sig(j,nz)/dz(nz)
 499     format(i4,10E12.3)
c      write(*,499) j,dc(j,nz)
          if (dc(j,nz).le.2.0) dc(j,nz) = 1000.0
c           dc(j,nz) = dc(j,nz) + 2.*an(j)*sig(j,nzp)/dz(nz)
 50   continue
c-Bottom boundary conditions:
...
c	  phoi = 4.0E7*rne(j,i) photoionization term
     ...
     r        (rjniz(j,i)-rjniz(j,i-1)))*deltel/dz(i)+att-reci 
...
c  999     format(1x,'res =',1p,e16.8,'   res0 =',e16.8)
 ...
 170        continue
  ...
  260    continue
         iconv = 1
  265    continue
       if (jscreen.eq.1) write(*,998) iter,diff,i,j,x(j,i),phi(j,i)
