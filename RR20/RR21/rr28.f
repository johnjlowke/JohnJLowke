c rr28.f has positive ions absorbed at cathode
c rr7.f adds Matlab files for particles
c rr25c.f corrects omission of "s" on eabs(j,i) line 521!
c rr25c.f has changes to multcoef
c rr25b.f signs changed; volts +ve; ez=-grad V is +ve; je is -ve!
c rr24.f has backward differences and corrction for negative rows
c rr22b.f has maximum particle densirt of 1.0E16
c rr22.f has electron density calculation made for each iteration of mm with linear rising voltage
c mom removed as all densiies set at arho(19); ener also not used
c rr21a.f space charge all i
...
c-----------------------------------------------------------------------
     ...
     r  cdz(0:mr,0:mz), cdr(0:mr,0:mz),cdez(0:mr,0:mz),cder(0:mr,0:mz)
      common/var/
     r  rlognetp(0:mr,0:mz),rlognetn(0:mr,0:mz),rrnet(0:mr,0:mz),
...
c          Calculation of electron densiy in each step of (1) by "elec"
c----------------------------------------------------------------------
         do 721 i=0,nz
         do 722 j=0,nr
            t1(j,i) = t(j,i)
...
c        timeel = 0.0
c        do 222 iii = 1, ntimesteps
c             call elec
c             curr = currm
c             nrrvtest = 1
c            call poiss(curr,volt,nrrvtest)
c             timeel = timeel + deltel
c 222    continue
c        irs=irs+1
c	if(incrs.gt.0)then
c	ncrs=ncrs+incrs
c	incrs=0
c	endif
c      write(*,226) mm, timeel, curr, volt
...
c plot files for MATLAB or IDL
      ...
      close(20)
       open(21,file='z.dat')
c      write(21,*) (z(i),i=ib,nzm)
      write(21,*) (z(i),i=ib,nz)
      close(21)
       open(22,file='t.dat')
       do 793 i=ib,nz
...
c tt and rr files for full plot from r = -R to r = +R; jjl
 ...
 819  continue
      ...
      do 821 i=ib,nz
          write(31,*) (rlognetp(j,i),j=1,jjend)
 821  continue
      close(31)
      open(33,file='netn.dat')
      do 822 i=ib,nz
          write(33,*) (rlognetn(j,i),j=1,jjend)
 822  continue
      close(33)
...
c include space charge sheath field at lower electrode
c      do 84 i=1,nz
c      do 85 j=0,nr
...
c          wir = -rmob*er(j,i)
cc          wire = 100*wir
c          if (wire.ge.1000000.) wire = 1000000.
c          if (wire.le.-1000000.) wire = -1000000.
c          if (wir.ge.100000.) wir = 100000.
c          if (wir.le.-100000.) wir = -100000.
c          if(er(j,i).le.0.0) then
c              rjnir(j,i) = wir*rni(j,i)
c              rjner(j,i)=wire*rne(j,i)-diff*(rne(j+1,i)-rne(j,i))*2/
c     r	          (dr(j+1)+dr(j))
c              rjnpr(j,i) = -wir*rnp(j+1,i)
c          else
c              rjnir(j,i) = wir*rni(j+1,i)
c              rjner(j,i)=wire*rne(j+1,i)-diff*(rne(j+1,i)-rne(j,i))*2/
c     r	          (dr(j+1)+dr(j))
c              rjnpr(j,i) = -wir*rnp(j,i)
c          endif
c          if (j.le.1) rjner(j,i) = 0.0
c Backward differences modified for reversed flow of current after current zero.
c        wiz = -rmob*ez(j,i) rmob is positive so for electrons and neg ions wiz and wize -ve
       do 193 i=1,nz
       do 194 j=1,nr
        wiz = -rmob*ez(j,i)
        wize = 100*wiz
...
c flow symmetric around central axial axis at r = 0 when j = 0
      do 212 i = 1,nz
          ...
          rjnpr(0,i) = rjnpr(1,i) 
 212  continue
c top electrode flow out or in at i=0 
      do 213 j = 1,nr
          rjnez(j,0) = rjnez(j,1)
          rjner(j,0) = rjner(j,1)
          rjniz(j,0) = rjniz(j,1) 
          ...
          rjnpz(j,0) = rjnpz(j,1)
          rjnpr(j,0) = rjnpr(j,1) 
c bottom boundary does not emit  particles
          rjnez(j,nzp) = 0.0  
          rjner(j,nzp) = 0.0  
          rjniz(j,nzp) = 0.0  
          rjnir(j,nzp) = 0.0 
          ...
          rjnpr(j,nzp) = 0.0  
 213  continue  
c bottom electrode does not emit electrons, neg ions or pos ions after currentzero
      do 214 j = 1,nr
       if (j.le.nra) then
       if (ez(j,i).ge.0.0) then
c            rjnpz(j,70) = 0.0
            rjnpz(j,70) =rjnpz(j,69)
       else
            rjnez(j,70) = 0.0
            rjniz(j,70) = 0.0
       endif
...
c Flow out = - grad (ne*velocity)
c          dne(j,i) =-((rjner(j,i)*ddrf-rjner(j-1,i)*ddrb)/dr(j) -
          dne(j,i) =-(rjner(j,i)*ddrf-rjner(j-1,i)*ddrb)/dr(j) -
     r        (rjnez(j,i)-rjnez(j,i-1))*deltel/dz(i)
c          if (dne(j,i).ge.rne(j,i)) dne(j,i) = rne(j,i)-1
c     r        +SS(j,i)+rion-rece-att 
...
c  use cp of gas even for ne electrodes
